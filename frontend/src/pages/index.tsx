...
import {
    ...
    fetchSunData,
    fetchSunHistory,
    calculatePlantHealthIndex,
    fetchHealthHistory,
    fetchForecast,
    fetchWaterRecommendation
} from '@/lib/api-client';
...
export default function Dashboard() {
    ...
    if (dashboardData.loading && !dashboardData.latestSensor) {
        return (
            <div
                className={`${geistSans.className} ${geistMono.className} bg-gray-50 text-gray-800 p-6 w-full h-screen flex items-center justify-center`}>
    ...
    return (
        <div
            className={`${geistSans.className} ${geistMono.className} bg-gray-50 text-gray-800 p-6 w-full min-h-screen`}>
            ...
            <div className="flex justify-between items-center mb-6 pb-2 border-b border-gray-200">
                ...
                <div className="flex items-center">
                    ...
                    >
                        <svg className="w-4 h-4 text-white dark:text-white" aria-hidden="true"
                    ...
                    <div className="h-48 w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={moistureRainChartData}>
                                ...
                                <Line type="monotone" dataKey="rain" stroke="#3b82f6" dot={false} strokeWidth={2}
                                      strokeDasharray={"3 3"}/>
                            </LineChart>
                        </ResponsiveContainer>
                    </div>
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={tempHumidityChartData}>
                                ...
                                <Line type="monotone" dataKey="temperature" stroke="#ef4444" dot={false}
                                      strokeWidth={2}/>
                                <Line type="monotone" dataKey="humidity" stroke="#38bdf8" dot={false} strokeWidth={2}
                                      strokeDasharray="3 3"/>
                            </LineChart>
                        </ResponsiveContainer>
                    </div>
                ...
                <div className="bg-white p-4 rounded-lg shadow-md">
                    ...
                    <div className="mt-6">
                        <div className="relative h-16 bg-gray-100 rounded-lg overflow-hidden">
                            ...
                            <div
                                className="absolute inset-0 bg-gradient-to-r from-indigo-900 via-blue-400 to-indigo-900">
                            ...
                            >
                                <div
                                    className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-3 h-3 bg-white rounded-full shadow-md border-2 border-blue-500"></div>
                            </div>
                ...
                <div className="bg-white p-4 rounded-lg shadow-md">
                    ...
                    <div className="mb-4">
                        {dashboardData.forecast.forecast ? (
                            <>
        <span
            className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(dashboardData.forecast.status)}`}>
          {toTitleCase(dashboardData.forecast.status.replace('_', ' '))}
        </span>

                                <div
                                    className="text-sm text-gray-600 mt-2">{dashboardData.forecast.summary.message}</div>

                                <div className="grid grid-cols-2 gap-4 mb-4 mt-4">
                                    <div className="text-center p-2 bg-gray-50 rounded-lg">
                                        <div className="text-sm text-gray-400 mb-1">Expected Rain</div>
                                        <div
                                            className="text-xl font-bold">{dashboardData.forecast.forecast.rain_mm} mm
                                        </div>
                                    </div>
                                    <div className="text-center p-2 bg-gray-50 rounded-lg">
                                        <div className="text-sm text-gray-400 mb-1">Condition</div>
                                        <div
                                            className="text-xl font-bold">{toTitleCase(dashboardData.forecast.forecast.description)}</div>
                                    </div>
                                    <div className="text-center p-2 bg-gray-50 rounded-lg">
                                    ...
                                    <div className="text-center p-2 bg-gray-50 rounded-lg">
                                        ...
                                        <div
                                            className="text-base font-medium text-gray-700">{dashboardData.forecast.forecast.datetime_local}</div>
                                    </div>
                                </div>
                            </>
                        ) : (
                ...
                <div className="bg-white p-4 rounded-lg shadow-md md:col-span-2 lg:col-span-3">
                    <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-200">
                        <div className="flex items-center">
                            ...
                            {alerts.length > 0 && (
                                <div
                                    className="ml-3 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs font-bold">
                                    {alerts.length}
                                </div>
                            )}
                        </div>
